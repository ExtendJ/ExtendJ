...
class Attributes {
    ...
    p.println("    " + attributes_count + " attributes:");
		for (int j = 0; j < attributes_count; j++) {
			...
			String attribute_name = ((CONSTANT_Utf8_Info) p.constantPool[attribute_name_index])
					.string();
	...
	public void innerClasses(TypeDecl typeDecl, TypeDecl outerTypeDecl) {
		...
		for (int i = 0; i < number_of_classes; i++) {
			...
			if(inner_class_info_index > 0 && outer_class_info_index > 0 && inner_name_index >  0) {
				CONSTANT_Class_Info inner_class_info = (CONSTANT_Class_Info) this.p.constantPool[inner_class_info_index];
				CONSTANT_Class_Info outer_class_info = (CONSTANT_Class_Info) this.p.constantPool[outer_class_info_index];
				...
				if (inner_name_index != 0) {
					inner_name = ((CONSTANT_Utf8_Info) this.p.constantPool[inner_name_index]).string();
				} else {
          ...
          typeDecl.setModifiers(Parser.modifiers(inner_class_access_flags & 0x041f));
				...
				if (outer_class_info.name().equals(this.p.classInfo.name())) {
					...
					p.println("Processing: " + inner_class_name);
					try {
            AST.ClassFile file = new AST.ClassFile(inner_class_name);
            if(file.exists()) {
						  Parser p = new Parser(file.buffer(), file.size());
    ...
    p.println("      " + number_of_exceptions + " exceptions:");
		for (int i = 0; i < number_of_exceptions; i++) {
			CONSTANT_Class_Info exception = (CONSTANT_Class_Info) this.p.constantPool[this.p.u2()];
	...
	private void constantValues() {
		...
		constantValue = this.p.constantPool[constantvalue_index];
	}
